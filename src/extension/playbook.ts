...
export const PARENT_PLAYBOOK = [
	...
	"5. A completion notice reading `collect timed out … the agent is still alive` is a progress signal, not a verdict: the child keeps running and will notify again when it truly finishes. Check `status` and the child's session tail before deciding to steer or continue — a `continue` while it is mid-turn appends to its queue rather than interrupting.",
	"6. Isolation is YOUR call. Pass `worktree: true` when another parent may write this repo, or when the child should ship via MR (own branch, do not touch the current checkout). Pass `worktree: false` to edit the current checkout in place. Omit it to use the role default (bundled worker isolates).",
